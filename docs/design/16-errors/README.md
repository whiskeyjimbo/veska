...
```jsonc
{
  "error": {
    ...
    "data": {
      "engram_code": "ErrXxx",     // stable string id; the catalogue key
...
The `engram_code` is the contract. The `message` is friendly
prose that may improve over time. Tooling MUST key on
`engram_code`, not `message`.
...
|---|---|
| 0 | Success. |
| 1 | Degraded — work completed but the result is partial or stale. `engram doctor` reports `status: degraded`. |
| 2 | Broken — work failed; the user must remediate. `engram doctor` reports `status: broken`. |
...
The `result` field carries the `engram_code` directly. SOLO-08
...
| `engram_code` | When | Remediation |
|---|---|---|
| `ErrCrashLoop` | `~/.engram/state/broken` marker present at start | `engram doctor reset-crash-loop` after investigation |
...
| `ErrUnsupportedFilesystem` | `~/.engram/` on NFS, eCryptfs, FUSE, or overlay-upper | move data dir; set `ENGRAM_HOME` |
| `ErrBackupRequired` | `[backup].required = true` and no verified backup found | `engram backup create`; restart |
...
Audit line: not written for the panic (the daemon is dying). Crash details land in `~/.engram/logs/daemon.log` and the breaker notification path (SOLO-03 §5.6).

### 3.3 MCP surface (JSON-RPC errors)

| `engram_code` | JSON-RPC | When | `context` payload | Remediation |
|---|---|---|---|---|
| `ErrDaemonNotRunning` | -32000 | Shim cannot reach socket and no supervisor is registered | `{"cli_command": "engram service install"}` | Install the service |
...
| `ErrRepoNotRegistered` | -32003 | Tool called with a `repo` that is not in `repos` | `{"repo_id_or_path": "..."}` | `engram repo add <path>` |
...
| `ErrEmbedFailed` | Embed row exhausted retries | `degraded_reasons:["embedding_failed"]`; `engram doctor post-promotion-queue retry --kind=embed` |
...
These are not MCP errors — they are *findings* or *degraded reasons*. The `engram_code` keys are stable so tooling can correlate.

### 3.5 Backup, restore, embedder swap

| `engram_code` | When | Exit | Remediation |
|---|---|---|---|
| `ErrBackupVerifyFailed` | `engram backup verify` integrity_check or FK check failed | 2 | The backup is unusable; create a new one |
| `ErrBackupCorrupt` | `engram doctor backup` finds the most recent backup unreadable | 2 | Same |
| `ErrRestoreDaemonRunning` | `engram backup restore` while daemon up | 2 | `engram daemon stop`; rerun |
...
| `engram_code` | When | Surface |
|---|---|---|
| `ErrDiskLow` | `engram doctor storage` free < 1 GiB | exit 1 (degraded); `messages[].code = "disk_low"` |
...
| `ErrFSEventsBudget` | macOS FSEvents per-mount budget exceeded | warn-level log; `engram doctor watcher` recommends polling |
...
|---|---|
| Adding a new `engram_code` | no — additive; tooling tolerates unknowns |
| Adding an optional field to `context` | no |
| Removing or renaming an `engram_code` | yes — minor version bump + CHANGELOG note |
| Repurposing an existing `engram_code` | yes |
| Changing the type of a `context` field | yes |

Tooling MUST tolerate unknown `engram_code` values (forward
...
- SOLO-13 §2 — `engram doctor` exit codes and `--json` schema.
