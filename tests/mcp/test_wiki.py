...
def test_get_hot_zone_requires_branch(mcp_client, repo_id):
    ok, text, _, _ = mcp_client.call("eng_get_hot_zone", {"repo_id": repo_id})
    assert not ok and "required" in text.lower()


def test_get_entry_points_requires_branch(mcp_client, repo_id):
    ok, text, _, _ = mcp_client.call("eng_get_entry_points", {"repo_id": repo_id})
    assert not ok and "required" in text.lower()
