...
// Open initialises the VectorStorage for the given engramHome directory.
//
// With the hnsw_native build tag: creates a UsearchStore, loads any persisted
// indexes from engramHome, and returns the store ready for use. The returned
...
func Open(engramHome string) (ports.VectorStorage, error) {
	return openNative(engramHome)
}
