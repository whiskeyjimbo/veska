...
// engramHome, and returns the store ready for use.
//
// If engramHome is empty or contains no vec-*.json files, Load is a no-op and
// an empty (but functional) store is returned.
//
// If Load fails, Destroy is called to release CGo memory before returning the
// error.
func openNative(engramHome string) (ports.VectorStorage, error) {
	store, err := NewUsearchStore()
	if err != nil {
		return nil, err
	}
	if err := store.Load(engramHome); err != nil {
		store.Destroy()
		return nil, err
	}
	return store, nil
