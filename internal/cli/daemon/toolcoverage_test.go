...
func blastFamily() []coverageTool {
	...
	return []coverageTool{
		{family: f, tool: "eng_get_blast_radius", bead: "solov2-6ups", run: func(t *testing.T) {
			...
			// seed=dirty: the merged tool also blasts the staging overlay. c1
			// commits Helper (callee) + Caller (CALLS Helper); stageDirtyEdit
			// simulates the fsnotify hot path writing an edited Helper into the
			// SHARED staging.Area the dirty path reads, so the callers blast must
			// surface Caller, the affected caller.
			assertBlastSeedDirty(t)
			// seed=diff: same shape, but the seed is a committed git diff.
			assertBlastSeedDiff(t)
		}},
	}
}

// assertBlastSeedDirty exercises eng_get_blast_radius with seed=dirty.
func assertBlastSeedDirty(t *testing.T) {
	t.Helper()
	root := initGitRepo(t)
	...
	if rpcErr != nil {
		t.Fatalf("eng_get_blast_radius seed=dirty: %v", rpcErr)
	}
	resp, ok := res.(mcp.BlastResponse)
	if !ok {
		t.Fatalf("eng_get_blast_radius seed=dirty: result type %T, want mcp.BlastResponse", res)
	}
...
// assertBlastSeedDiff exercises eng_get_blast_radius with seed=diff. c1 seeds
// Helper (callee) + Caller (CALLS Helper); c2 edits ONLY helper.go. Diffing
// HEAD~1..HEAD changes {helper.go} -> its node Helper -> the callers blast must
// surface Caller, the affected caller.
func assertBlastSeedDiff(t *testing.T) {
	t.Helper()
	root := initGitRepo(t)
	writeRepoFile(t, root, "helper.go", "package p\n\nfunc Helper() int { return 1 }\n")
	writeRepoFile(t, root, "caller.go", "package p\n\nfunc Caller() int { return Helper() }\n")
	...
	if rpcErr != nil {
		t.Fatalf("eng_get_blast_radius seed=diff: %v", rpcErr)
	}
	resp, ok := res.(mcp.BlastResponse)
	if !ok {
		t.Fatalf("eng_get_blast_radius seed=diff: result type %T, want mcp.BlastResponse", res)
	}
