...
//	  This is the default and matches the definition of
//	  blast radius.
...
type Service struct {
	...
	// (including a negative one, which disables the gate).
...
type Options struct {
	...
	// motivation: framework registry nodes like cobra's
...
func (s *Service) DiffOf(ctx context.Context, repoID, branch, repoRoot string, changedFiles ChangedFilesFunc, opts Options) (Response, error) {
	...
	for _, fp := range files {
		...
		// path, so a diff path feeds NodesInFile directly. An
...
func (s *Service) DirtyOf(ctx context.Context, repoID, branch string, opts Options) (Response, error) {
	...
	// was actually staged (matching the 4.4 contract and the Of/DiffOf
	// paths), not merely because this is the dirty view. A clean working tree
	// yields no seeds and reports false.
