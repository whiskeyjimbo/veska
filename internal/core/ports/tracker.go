...
// Task is the ports-layer view of a unit of tracked work. It carries only the
// fields needed by application-layer callers; the full domain.Task may hold
// richer data that is not surfaced here.
type Task struct {
...
type Tracker interface {
	...
	ActiveTask(ctx context.Context, repoID string) (*Task, error)
	...
	RecentTasks(ctx context.Context, repoID string, limit int) ([]Task, error)
}
