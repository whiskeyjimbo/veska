# end-to-end SQLite driver comparison
...
(which dbbench already settled in mattn's favor). The shim
...
| Cold-scan hugo (223 k LOC / 899 Go files) | Same path on a real 200 k+ LOC public repo. Surfaced mid-bench (UNIQUE-PK collision on function-local types + multi-init protobuf files); fix landed before this row. | 197.5 s total (promote 193.6 s) | **121.1 s total (promote 117.2 s)** | **1.63× - ~76 s saved on a single cold scan** |
...
- The mattn-p95-promotion_tx regression observed in dbbench
