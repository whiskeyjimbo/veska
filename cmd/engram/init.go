...
//  1. Creates the ~/.engram/ directory layout (logs/, cache/, state/).
...
func runInit(ctx context.Context, deps initDeps, yes bool, out io.Writer) error {
	...
	for _, sub := range []string{"logs", "cache", "state"} {
		if err := os.MkdirAll(filepath.Join(deps.engramHome, sub), 0o755); err != nil {
	...
	// ── 3. 6-line summary ────────────────────────────────────────────────────
	fmt.Fprintln(out, "engram initialized")
	fmt.Fprintf(out, "data:     %s\n", deps.engramHome)
	fmt.Fprintf(out, "embedder: %s (%s @ %s)\n", result.Status, defaultModelName, defaultOllamaURL)
	fmt.Fprintln(out, "service:  not installed (run: engram service install)")
	fmt.Fprintln(out, "repo:     not added (run: engram workspace add .)")
...
func initCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "init",
		Short:        "First-run setup: create ~/.engram/ layout, probe embedder, and print summary",
		SilenceUsage: true,
