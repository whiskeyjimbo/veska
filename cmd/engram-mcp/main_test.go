...
func TestSocketMissingJSONError(t *testing.T) {
	const sockPath = "/tmp/engram-test-nonexistent-12345.sock"
	...
	if resp.Error.Data.CLICommand != "engram service start" {
		t.Errorf("error.data.cli_command = %q; want \"engram service start\"", resp.Error.Data.CLICommand)
	}
